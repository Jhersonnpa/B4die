...
<head>
    ...
    <link rel="stylesheet" href="<?= base_url('css/editarAct.css')?>">
...
<body onload="getLocation()">
    ...
    <div class="container">
        ...
        <form action='<?=base_url('/editadaActivitat')?>' method='post' enctype='multipart/form-data' class="form">
            <?= csrf_field() ?>
                    <?php
                    $activitat['id']=isset($activitat['id'])? $activitat['id']:"";
                    $activitat['nom']=isset($activitat['nom'])? $activitat['nom']:"";
                    $activitat['descripcio']=isset($activitat['descripcio'])? $activitat['descripcio']:"";
                    $activitat['categoria']=isset($activitat['categoria'])? $activitat['categoria']:"";
                    $activitat['subcategoria']=isset($activitat['subcategoria'])? $activitat['subcategoria']:"";
                    $activitat['pais']=isset($activitat['pais'])? $activitat['pais']:"";
                    $activitat['ciutat']=isset($activitat['ciutat'])? $activitat['ciutat']:"";
                    $activitat['latitud']=isset($activitat['latitud'])? $activitat['latitud']:"";
                    $activitat['longitud']=isset($activitat['longitud'])? $activitat['longitud']:"";
                    $activitat['dificultat']=isset($activitat['dificultat'])? $activitat['dificultat']:"";
                    $activitat['tipo_img']=isset($activitat['tipo_img'])? $activitat['tipo_img']:"";
                    $activitat['precio']=isset($activitat['precio'])? $activitat['precio']:"";
                    $activitat['url_empresa']=isset($activitat['url_empresa'])? $activitat['url_empresa']:"";
                    ...
                    ?>
                <label for="nom">Nombre actividad</label>
                    <input required type="text" name="nom" id="nom" value="<?= $activitat['nom']?>">
                    
                    <?php
                    if (!empty($validation)) {
                        if ($validation->getError('descripcio')) {
                            echo $validation->getError('descripcio');
                        }    
                    }
                    ?>
                <label for="descripcio">Descripción actividad</label>
                    <textarea name="descripcio" id="descripcio"><?=  $activitat['descripcio']?></textarea>
                
                    <?php
                    if (!empty($validation)) {
                        if ($validation->getError('categoria')) {
                ...
                <label for="categoria">Categoria</label>
                    ...
                    <?php
                    if (!empty($validation)) {
                        if ($validation->getError('subcategoria')) {
                            echo $validation->getError('subcategoria');
                        }    
                    }
                    ?>
                ...
                <label for="pais">Pais</label>
                    ...
                    <?php
                    if (!empty($validation)) {
                        if ($validation->getError('ciutat')) {
                            echo $validation->getError('ciutat');
                        }    
                    }
                    ?>
                <label for="ciutat">Ciudad</label>
                    <input required type="text" name="ciutat" id="ciutat" value="<?= $activitat['ciutat']?>">
                
                    <?php
                    if (!empty($validation)) {
                        if ($validation->getError('latitud')) {
                            echo $validation->getError('latitud');
                        }    
                    }
                    ?>
                <label for="latitud">Latitud</label>
                    <input required type="number" step="any" name="latitud" value="<?= $activitat['latitud']?>"/>
                
                    <?php
                    if (!empty($validation)) {
                        if ($validation->getError('longitud')) {
                            echo $validation->getError('longitud');
                        }    
                    }
                    ?>
                <label for="longitud">Longitud</label>
                    <input required type="number" step="any" name="longitud" value="<?= $activitat['longitud']?>"/>
                
                    <?php
                    if (!empty($validation)) {
                        if ($validation->getError('dificultat')) {
                            echo $validation->getError('dificultat');
                        }    
                    }
                    ?>
                <label for="dificultat">Dificultad</label>
                <input required type="number" step="any" name="dificultat" value="<?= $activitat['dificultat']?>"/>
                ...
                <input type="hidden" name="tipo_img" value="<?= $activitat['tipo_img']?>">
                <input type="hidden" name="id" value="<?= $activitat['id']?>">
                <?php 
                if (isset($activitat['img'])) {
                    $session->setFlashdata('img2',$activitat['img']);
                }
                ...
                <?php
                    if (!empty($validation)) {
                        if ($validation->getError('precio')) {
                            echo $validation->getError('precio');
                        }    
                    }
                    ?>
                <label for="precio">Precio</label>
                <input required type="text" name="precio" id="precio" value="<?= $activitat['precio']?>">
                
                <?php
                    if (!empty($validation)) {
                        if ($validation->getError('url_empresa')) {
                            echo $validation->getError('url_empresa');
                        }    
                    }
                    ?>
                <label for="url_empresa">URL Empresa</label>
                    <input required type="text" name="url_empresa" id="url_empresa" value="<?= $activitat['url_empresa']?>">
